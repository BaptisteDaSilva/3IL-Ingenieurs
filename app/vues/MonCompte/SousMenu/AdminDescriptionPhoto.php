...
<div id="panel_adminPhoto" class="panel-body panel-photo">
	<div class="row">
		...
		<div class="col-md-8">
			...
			<form method="post"
				action="/Administration/modifierDescriptionPhoto/">
				<input type="hidden" name="idLangue" value="<?= $langue->getId() ?>" />
				<div class="photos">                            
                <?php foreach ($this->photos as $photo) { $pName = Photo::getName($photo); ?>
                    <div class="photo">
						<img src="<?= PHOTOS . $pName ?>" alt="<?= $pName ?>" /> <input
							class="form-control" id="<?= $pName ?>" type="text"
							name="photos[<?= $pName ?>]" placeholder="Ajouter description"
							value="<?= Photo::getDescription($pName, $langue->getId()) ?>" />
					</div>
                <?php } ?>        
                </div>
				<div>
					<input type="submit" name="submitLangue" value="Modifier"
						class="btn mon-btn">
				</div>
			</form>
            <?php
