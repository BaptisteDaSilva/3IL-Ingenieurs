	...
	<div class="row">
        ...
        <?php } else { ?>
            <form method="post" action="/MonCompte/modifierAvatar/">
				<input type="hidden" name="nomAvatar" id="nomAvatar"
					value="<?= $_SESSION['util']->getNomAvatar() ?>" />
				<div class="avatars">                            
                <?php
									foreach ( $this->avatars as &$avatar ) {
										if ($avatar->getNom () == $_SESSION ['util']->getNomAvatar ()) {
											?>
                        <div class="avatar">
						<img class="selected" src="<?= AVATAR . $avatar->getNom() ?>"
							alt="<?= $avatar->getNom() ?>" />
					</div>
                    <?php } else { ?>                
                        <div class="avatar">
				...
				<div>
					<input type="submit" name="submitAvatar" value="Enregistrer"
						class="btn mon-btn">
				</div>
			</form>
        <?php
