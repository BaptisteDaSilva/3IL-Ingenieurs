	...
	<div class="row">
		...
		<?php } else { ?>
            <form method="post" action="/MonCompte/modifierLangue/">
				<input type="hidden" name="nomLangue" id="nomLangue"
					value="<?= $_SESSION['util']->getNomLangue() ?>" />
				<div class="drapeaux">  
                <?php
			foreach ( $this->langues as &$langue ) {
				if ($langue->getNom () == $_SESSION ['util']->getNomLangue ()) {
					?>
                    	<div class="drapeau">
						<img class="selected"
							src="<?= DRAPEAU . $langue->getNomDrapeau() ?>"
							alt="<?= $langue->getNom() ?>" />
						<p><?= $langue->getNom() ?></p>
					</div>
                <?php } else { ?>
			...
			?>
				...
				<div>
					<input type="submit" name="submitLangue" value="Enregistrer"
						class="btn mon-btn">
				</div>
			</form>
        <?php
