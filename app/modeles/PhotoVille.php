...
class PhotoVille extends Modele
{
    ...
    public static function getPhotosVille($ville, $nbPhotos)
    {
        ...
        // Prépare la requête
        $requete = self::getBaseDeDonnees()->getCnxBD()->prepare(self::RQT_PHOTOS_VILLE);
        ...
        for ($i = 0; $i < count($photosBD); $i ++) {
            $photos[$i] = new PhotoVille($photosBD[$i]->idphoto, $photosBD[$i]->nom, $ville);
        }
    ...
    {
        return PHOTOS_VILLES . $this->ville->getNumInsee() . '/' . $this->id . '.jpg';
    }
}