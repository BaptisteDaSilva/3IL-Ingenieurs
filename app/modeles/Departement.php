...
class Departement extends Modele
{
     ...
     *            le chef-lieu du dpartement.
     * @param array $villes
     *            la liste des villes du département.
     * @param string $desc
     *            la description du département.
     */
    public function __construct($numDep, $nom, Region $region = null, Ville $chefLieu = null, $villes = null, $desc = null)
    {
        $this->numDep = $numDep;
        $this->nom = $nom;
        $this->region = $region;
    ...
    public static function getDepartement($numDep)
    {
        ...
        // Prépare la requête
        $requete = self::getBaseDeDonnees()->getCnxBD()->prepare(self::RQT_DETAILS_DEPARTEMENTS);
        ...
        return $dep ? new Departement($dep->numdep, $dep->nomdep, new Region($dep->numreg, $dep->nomreg), new Ville($dep->numinsee, $dep->nomcheflieu), $villes, $dep->des) : null;
    }
    ...
    private static function getListeVilles($numDep)
    {
        ...
        // Prépare la requête
        $requete = self::getBaseDeDonnees()->getCnxBD()->prepare(self::RQT_VILLE_DEPS);
    ...
    public static function setDesc($numDep, $desc)
    {
        ...
        // Prépare la requête
        $requete = self::getBaseDeDonnees()->getCnxBD()->prepare(self::RQT_MODIF_DEP);
